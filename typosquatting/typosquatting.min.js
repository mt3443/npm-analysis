...
function detect_typosquatting(package_name) {
    ...
    return run_tests(package_name);
}

function crosscheck(packages) {
    let total = packages.length;
    let caught = 0;
    let missed = [];

    for (let package of packages) {
        if (run_tests(package) != false) {
            caught++;
        } else {
            missed.push(package);
        }
    ...
    console.log(caught / total);
    console.log(missed);

}

module.exports = {detect_typosquatting: detect_typosquatting, crosscheck: crosscheck}