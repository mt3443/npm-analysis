        ...
        if (process.stdout.isTTY) {
            switch (clazz) {
                case 'low' :
                    ...
                    break;
                case 'medium':
                    str = '\033[93m' + str + '\033[0m';
                    break;
                case 'high':
                    str = '\033[92m' + str + '\033[0m';
                    break;
            }
        }
        return str;
 ...
 */
function RadicReport(opts) {
    Report.call(this);
    this.opts = opts || {};
    this.opts.useFrontMatter = this.opts.useFrontMatter || true;
    this.opts.frontMatterPath = this.opts.frontMatterPath || path.resolve(__dirname, 'templates', 'front-matter.yml');
    this.opts.templateDir = this.opts.templateDir || path.resolve(__dirname, 'templates');
    this.opts.vendorDir = this.opts.vendorDir || path.resolve(__dirname, 'vendor');
    ...
    this.templateFor = function (name) {
        return handlebars.compile(fs.readFileSync(path.resolve(this.opts.templateDir, name + '.hbs'), 'utf8'));
    };

    this.frontMatter = '';
    if(this.opts.useFrontMatter === true){
        this.frontMatter = "---\n" + fs.readFileSync(this.opts.frontMatterPath, 'utf8') + "\n---\n";
    }

    this.headerTemplate = this.templateFor('head');
    this.footerTemplate = this.templateFor('foot');
}

RadicReport.TYPE = 'radic';
util.inherits(RadicReport, Report);

Report.mix(RadicReport, {
